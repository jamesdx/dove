...
## Story 是否覆盖了 需求检查
让我分析一下当前的用户故事覆盖情况，并确定是否还有遗漏的需求点。


## 
请分析一下当前的"非功能性需求（NFR）信息模板"覆盖情况，并确定是否还有遗漏的需求点。

# Design Overall Process

## 给个部分生成具体步骤，然后利用具体步骤 让AI生成生成这部分内容的 Prompt 
2.1 定义的活动，是不是也会有先后顺序，绘制个流程图，
## 让AI生成生成这部分内容的 Prompt
你现在是个天才Prompt Engineer, 你如何写一个 Prompt 能让AI  去完成 2.1 活动流程定义的的动作，然后输出内容是 2.1 定义的输出内容，而且是专业、权威、企业企业级的

## Promt 优化，加入检查环节
我想你在这个prompt 一个步骤，就是成功输出后，要做自我检查，是否完全按照文档格式输出、是否符合输出要求。生成内容是不否是专业的、企业级的、完全覆盖需求的，尤其是完全覆盖需求
...
# Generation Design Document
你是一个有着30年实际工作经验的架构师和DDD专家，请根据 文档中 "预期输出"、"输出质量要求" 内容，给每种交付物 生成一个 Markdown 格式的的模版，模版的结构要全面、专业、企业级的，而且要完全覆盖需求； 例如："领域划分文档"，文档名字规则： dove-login-design-${design-type}-${delivery-document-type}.md 

# 完善 模版
已经很好了，请给每个条元素添加一个 不少于200字的说明，用于说明这部分内，方便AI或者人去理解。第二，每个元素要添加一个例子。 例如你要给 "2.1 上下文关系图" 加入如上内容 
