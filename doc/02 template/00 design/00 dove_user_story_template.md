...
---
## 4. 业务优先级 & 核心价值 (Business Priority & Core Value)

> 在此部分，说明本用户故事在全局业务中的优先级，以及它是否属于 **核心域 (Core Domain)**、**支撑域 (Supporting Domain)** 还是 **通用域 (Generic Subdomain)**。这样可方便在 DDD 战略设计中做边界划分、明确资源投入。

- **优先级 & 投资价值**:
  - [示例] 此功能是订单系统的关键流程，属核心域，必须优先在首个版本实现。
  - [示例] 若属辅助或支撑功能，可排在后面迭代。

- **核心/支撑/通用 域划分**:
  - [示例] 核心域：涉及直接增值或差异化竞争力的部分。
  - 支撑域：系统需要，但不是核心竞争力（如促销、积分）。
  - 通用域：可在市面上复用的通用功能（如支付网关集成、通用身份认证）。

- **资源与团队投入**:
  - [示例] 由于此为核心域，需投入最资深团队成员与最多资源进行开发与测试。

---

## 5. 领域模型映射 (Domain Mapping)
...
## 7. 完整流程 (Main Flow & Alternate Flows)
在此部分，需要既有**流程图**，也要有**文字描述**，以便更直观地理解。
### 7.1 流程图 (Flow Diagram)
```mermaid
flowchart LR
    %% 仅示例，可根据实际流程自定义
...
## 14. 角色及权限需求 (Roles & Permissions)
在此小节中，列出本用户故事所涉及到的角色以及他们可执行的操作、受限行为或需满足的安全策略。 这些内容对于后续在 API 设计 和 安全设计（鉴权、RBAC/ABAC 等）中极其重要。
...
- 示例:
 - 1.	买家 (Buyer)
    ...
    -  不可访问管理端接口，不可查看他人订单。
 - 2.	管理员 (Admin)
    - 可以查看并管理所有订单的支付日志、错误原因；
    - 可以触发退款或重新扣款操作；
    - 可以配置支付方式、启用/禁用不同的支付网关。
 - 3.	客服 (Customer Service)
    - 可以查看订单的支付状态，协助用户查问题，但无权进行真实扣款或修改支付配置；
    - 可发起 “重试支付” 申请，但需管理员最终确认。
 - 4.	系统运维 (Ops)
    - 可以查看整个系统的支付监控和日志流量；
    - 配合观察高并发场景下的支付负载，并进行扩容；
    - 不可操作用户具体订单数据（仅对底层服务/基础设施操作）。

如果需要更细粒度，可写成「角色 → 权限点（Create/Read/Update/Delete）」，为后续 RBAC 或 ABAC 实现提供明确映射。