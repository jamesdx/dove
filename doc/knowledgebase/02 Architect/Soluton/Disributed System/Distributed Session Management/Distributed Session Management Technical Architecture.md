...
###### 2.3.1 用户认证流程
1. **登录认证流程**
   - 用户通过Browser/MobileApp/ThirdParty发起登录请求
   - 请求经过LB转发至WAF进行安全检查
   - WAF转发至API Gateway进行统一处理
   - Gateway调用TokenValidate进行令牌验证
   - 路由至AuthService进行身份认证
   - AuthService调用OAuth/JWT服务生成令牌
   - 创建分布式会话并存储在Redis集群
   - 返回认证结果和访问令牌
...
2. **Token验证流程**
   - 客户端请求携带Token访问服务
   - Gateway获取请求中的Token
   - TokenValidate组件验证Token有效性
   - 调用AuthorizationService进行权限验证
   - 验证通过后转发至目标服务
   - 服务处理业务逻辑并返回结果
